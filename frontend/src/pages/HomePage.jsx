...
const HomePage = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 space-y-12 py-12">
      ...
      <div className="bg-neutral-800/80 backdrop-blur-md p-4 md:p-6 rounded-[2.5rem] shadow-xl shadow-black/50 border border-neutral-700 flex flex-col md:flex-row gap-4 max-w-4xl mx-auto relative z-10 hover:shadow-2xl hover:shadow-yellow-400/10 transition-shadow duration-500">
        ...
        <button 
          ...
          disabled={loading}
          className="bg-gradient-to-br from-yellow-500 to-yellow-600 p-4 md:px-10 rounded-2xl text-black shadow-lg shadow-yellow-500/30 hover:shadow-yellow-500/50 hover:from-yellow-400 hover:to-yellow-500 transition-all active:scale-95 disabled:opacity-70 flex justify-center items-center group"
        >
          {loading ? (
      ...
      <div className="space-y-8 max-w-4xl mx-auto">
        {expertiza && (
          <div className="bg-gradient-to-r from-neutral-800 to-neutral-900 border-l-4 border-yellow-500 p-6 rounded-3xl shadow-md flex gap-4 items-start shadow-black/50">
            <div className="bg-yellow-400/10 p-2 rounded-full shadow-sm text-yellow-400">
              <Sparkles className="animate-pulse" size={20} />
            </div>
            <div>
              <h4 className="font-black text-neutral-300 text-xs uppercase tracking-widest mb-1">
                Expertiză Generală Platforma Feroviară (AI)
              </h4>
              <p className="text-neutral-100 text-sm font-semibold leading-relaxed">
                {expertiza}
              </p>
            </div>
          </div>
        ...
        {rute.length > 0 ? rute.map((ruta) => (
          <div key={ruta.idRuta} className="bg-neutral-800/80 backdrop-blur-sm p-6 md:p-8 rounded-[3rem] shadow-lg shadow-black/50 border border-neutral-700 hover:-translate-y-2 hover:shadow-2xl hover:shadow-yellow-400/15 transition-all duration-500 ease-out group">
            ...
            <div className="flex justify-between items-center mb-8 border-b border-neutral-700 pb-4">
              ...
              <span className="bg-yellow-400/10 text-yellow-400 border border-yellow-400/20 px-4 py-1.5 rounded-full font-bold italic uppercase text-xs tracking-wider shadow-sm">
                {ruta?.tren?.tipTren}
              </span>
            </div>

            {ruta.recomandareAi && (
              <div className="mb-8 p-5 md:p-6 bg-gradient-to-br from-neutral-900/80 to-neutral-800/80 backdrop-blur-sm border-l-4 border-yellow-500 rounded-r-3xl flex gap-4 items-start shadow-inner">
                <div className="bg-yellow-400/10 p-2 rounded-full shadow-sm">
                  <Sparkles className="text-yellow-400 shrink-0" size={20} />
                </div>
                <div className="text-sm text-neutral-200 leading-relaxed font-medium">
                  <span className="font-black text-yellow-500 uppercase text-[10px] block mb-1.5 tracking-widest">
                    Analiză Smart AI
                  </span>
                  {ruta.recomandareAi}
                </div>
            ...
            <div className="flex justify-between items-center text-center bg-gradient-to-r from-neutral-900 to-neutral-800/50 p-6 rounded-3xl border border-neutral-700 group-hover:border-yellow-500/30 transition-colors">
              <div className="flex-1">
                <p className="text-3xl font-black text-neutral-100 drop-shadow-sm">{getOra(ruta.opriri, plecare, 'plecare')}</p>
                <p className="text-xs font-bold text-neutral-500 uppercase tracking-widest mt-1">{plecare || "Sursa"}</p>
              </div>
              
              <div className="flex-[2] flex flex-col items-center justify-center px-4 md:px-8">
                <div className="w-full flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-yellow-500/50"></div>
                  <div className="flex-1 h-[2px] bg-gradient-to-r from-yellow-500/50 via-neutral-500 to-yellow-500/50 relative rounded-full">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs font-bold text-neutral-500 uppercase tracking-widest mt-1">{destinatie || "Destinatie"}</p>
              </div>
            </div>
            
          </div>
        )) : (
          <div className="text-center py-24 bg-neutral-800/50 backdrop-blur-sm border-4 border-dashed border-neutral-700 rounded-[3rem] text-neutral-500 font-medium italic shadow-inner">
